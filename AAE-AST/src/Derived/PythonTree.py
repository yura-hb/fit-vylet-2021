...
class PythonTree(Tree):
  ...
  def __init__(self, json):
    ...
    root = PythonTree.__make_node('0', json[0], json)
  ...
  def __make_node(id: str, item_json: dict, json: list) -> list[Node]:
    ...
    map = {
      r'Module': PythonTree.__make_module_node,
      r'(FuncDef)|(FunctionDef)': PythonTree.__make_func_node,
      r'ClassDef': PythonTree.__make_class_node,
      r'(Str)|(Num)': PythonTree.__make_constant_node,
      r'(Import)|(ImportFrom)': PythonTree.__make_import_node,
    ...
    for regex, func in map.items():
      if re.match(regex, type):
        return list(func(id, item_json, json))

    node = Dummy(None, repr(item_json), [])

    node.children = list(PythonTree.__traverse_children(id, item_json, json))

    return [node]

  @staticmethod
  def __make_module_node(id: str, item_json: dict, json: list):
    attributes = []
    body = []
  ...
  @staticmethod
  def __make_func_node(id: str, item_json: dict, json: list):
    ...
    if PythonTree._children in item_json.keys():
      for child in item_json[PythonTree._children]:
        ...
        if node[PythonTree._type] == 'body':
          if PythonTree._children in node.keys():
            for node_child in node[PythonTree._children]:
              body += PythonTree.__make_node(node_child, json[node_child], json)
  ...
  @staticmethod
  def __make_class_node(id: str, item_json: dict, json: list):
    name = item_json[PythonTree._value]
  ...
  @staticmethod
  def __make_constant_node(id: str, item_json: dict, json: list):
    return [Constant(None, id, item_json[PythonTree._type].lower(), item_json.get(PythonTree._value))]

  @staticmethod
  def __make_import_node(id: str, item_json: dict, json: list):
    ...
    if item_json[PythonTree._type] == 'Import':
      ...
      yield Import(None, id, alias = import_alias[PythonTree._value])
    ...
    if item_json[PythonTree._type] == 'ImportFrom':
      for child in item_json[PythonTree._children]:
        ...
        yield Import(None, id, fromModule = value, alias = import_alias[PythonTree._value])

    yield Dummy(None, repr(item_json), [])

  @staticmethod
  def __traverse_children(id: str, item_json: dict, json: list):
    if PythonTree._children in item_json.keys():
      for node_child in item_json[PythonTree._children]:
        yield PythonTree.__make_node(str(node_child), json[node_child], json)
